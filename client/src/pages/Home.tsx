    ...
    <PageFrame>
      ...
      <section className="page-section studio-highlights">
        ...
        <div className="editorial-grid">
          ...
          <article className="art-feature"><img src={initialArtworks[0].image} alt={locale === "en" ? "Watercolour landscape study" : "水彩風景習作"} /><div><p className="content-label">{t(initialArtworks[0].medium, locale)} · {initialArtworks[0].year}</p><h3>{t(initialArtworks[0].title, locale)}</h3><Link href="/art" className="text-link">{locale === "en" ? "Visit the gallery" : "前往作品集"} <ArrowRight size={14} /></Link></div></article>
        </div>
