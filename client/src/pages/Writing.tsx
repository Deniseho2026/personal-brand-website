...
export default function Writing() {
  ...
  return <PageFrame>
    ...
    <section className="page-section article-list"><div className="article-list-meta"><p>{locale === "en" ? "Recent notes" : "近期筆記"}</p><Bookmark size={17} /></div>{initialArticles.filter(article => article.availableLanguages.includes(locale)).map((article, index) => <article className="article-card" key={article.slug}><div className="article-image"><img src={article.image} alt={articleText(article.title, locale)} /></div><div className="article-copy"><p className="content-label">{articleText(article.category, locale)}</p><Link href={`/writing/${article.slug}`} className="text-link">{locale === "en" ? "Read note" : "閱讀筆記"} <ArrowRight size={14} /></Link></div><span className="article-index">{String(index + 1).padStart(2, "0")}</span></article>)}</section>
  </PageFrame>;
}
